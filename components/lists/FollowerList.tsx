    ...
    <div className="bg-zinc-900 border border-zinc-700/50 rounded-xl shadow-2xl overflow-hidden transition-all duration-300 w-full max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
      <div className="p-4 border-b border-zinc-700/50 flex justify-between items-center bg-gradient-to-r from-indigo-600/20 to-fuchsia-600/20">
        <h2 className="text-xl font-bold text-white flex items-center">
          ...
          <span>Followers</span>
          <span className="ml-2 bg-indigo-500/30 text-white px-2 py-0.5 rounded-full text-xs">{followerList.length}</span>
        </h2>
      ...
      <div className="p-3 border-b border-zinc-700/50">
        <div className="relative">
          ...
          <input type="text" placeholder="Search followers..." className="w-full bg-zinc-800 border border-zinc-700 rounded-lg py-2 pl-10 pr-4 text-zinc-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/50" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      </div>
      ...
      <div className="overflow-y-auto grow">
        {filteredFollowers.length > 0 ? (
              ...
              <li key={follower.id} className="p-4 hover:bg-zinc-800/50 transition-colors">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Image src={getAvatar(follower)} alt="profile_img" height={48} width={48} className="rounded-full border border-zinc-700" />
                    <div>
          ...
          <div className="flex flex-col items-center justify-center h-32 text-zinc-500"><p>No followers found</p></div>
        )}
      </div>
    </div>
  );
